...
import cn.hi028.android.highcommunity.adapter.HuiGdBuyAdapter;
...
public class HuiLifeSuppBuyFrag extends BaseFragment {
    ...
    public static List<GdCarBean> listData;
    int type = 0;
    public HuiSuppOrderParams orderParams;
    HSuppGdDefBean data;
    PopupWindow waitPop;
    HuiGdBuyAdapter adapter;

    @AfterViews
    void initView() {
        adapter = new HuiGdBuyAdapter(this);
        ...
        if (type == 0) {
            GdCarBean gdParams = new GdCarBean();
            gdParams.setGid(entyBean.getGid());
            gdParams.setStorage(entyBean.getNumber());
            gdParams.setGoods_name(entyBean.getGoods_name());
            gdParams.setPic(entyBean.getPic().get(0).getSmall());
            gdParams.setPrice(entyBean.getPrice());
            gdParams.setNum(1);
            List<GdCarBean> dataGdParams = new ArrayList<GdCarBean>();
            ...
            HTTPHelper.GetHuiSuppGoodsMsg(mIbpi, HighCommunityApplication.mUserInfo.getId() + "", listData.get(0).getGid() + "");
        }
    }
